...
export default function StatsTable() {
  console.log("STATS_TABLE", STATS_TABLE);
  return (
    <Container className="pt-12">
      ...
      <div className="gg overflow-auto py-4">
        <div className="flex gap-x-8 md:gap-x-24">
          <StatTable data={STATS_TABLE.slice(0, 3)} indexStart={1} />
          <StatTable data={STATS_TABLE.slice(3, 6)} indexStart={4} />
        </div>
      </div>
    </Container>
  )
      ...
      <tbody className="text-left text-sm font-semibold text-black md:text-base">
        {data.map((item, i) => (
          <tr className="cursor-pointer hover:bg-slate-100 [&>td]:px-4 [&>td]:py-3 first:[&>td]:pl-2" key={i + indexStart}>
            ...
            <td onClick={() => { usenavigate(`property/${i + 1}`); }}>
              <div className="flex items-center gap-x-3 pr-6 md:gap-x-6">
                <div className="relative aspect-square w-14 overflow-hidden rounded-xl border lg:w-[4.25rem]">
                  <img src={`${item.image}`} className="absolute inset-0 h-full w-full object-cover object-center" />
                </div>
                <div>
                  <p>{item.title}</p>
            ...
            <td className="hidden md:table-cell">{item.price} ETH</td>
            <td className="text-right">{item.size} sqft</td>
          </tr>
        ))}
      </tbody>
    </table>
